...
| Stack（LIFO：嵌套/撤销/等搭档） | **394**（85%） | — | 94 迭代遍历（15%） | — | — |
| Queue（FIFO：到达序/滑动计数） | **622** · 346° · 933（95%） | — | —（102 记在 Graph 圈当 frontier） | — | — |
| Heap（优先序：随时拿最值） | **215 · 295** · 703 · **23**（70%） | — | — | **743**（10%，与 greedy 行同题双料） | 253°（20%，会议室族） |
| TreeMap（全序：邻居/rank） | **352**（60%） | — | — | — | 729 · 715°（40%，日历/区间簿） |

> 在线/stream = 1-d 的受限变体（随机访问被没收）→ sort/二分/对撞指针出局 → 只剩「边收边维护」功率谱，正好是上面四行从便宜到贵各选一档：346 队列 → 239 单调队列 → 703 定容堆 → 295/480 双堆 → 352 TreeMap。

| 1-d algo（走法） | 1-d · string | 链表 | 区间 |
|---|---|---|---|
| 滑窗 | **3** | — | — |
| 双指针（含中心扩展） | **167 · 5** | — | — |
| 快慢指针 | **287**（数组当链表） | 141 / 142 | — |
| 单调栈 / 单调队列 | **739** | — | — |

| Graph algo（走法） | 2d（图眼镜） | 树 | 图 |
|---|---|---|---|
| BFS / DFS | **1091 · 200** | **102** | — |
| 回溯 | **79** | **78**（输入 1-d，决策树自造） | — |
